// Top section of the Search page — large two-field search + results header (sort + view toggle)

const SearchHero = ({ lang, query, setQuery, where, setWhere }) => {
    ...
    <section style={{
      ...
      <div style={{
        ...
        <button style={{
          ...
          <window.Icon.search size={14} /> {c.searchCta}
        </button>
      </div>
    </section>
  );
...
const SearchField = ({ icon, label, placeholder, value, onChange, right }) => (
  <div style={{
    flex: 1, minWidth: 200,
...
const ResultsHeader = ({ lang, count, total, sort, setSort, view, setView, dense, setDense }) => {
  const c = window.SEARCH_COPY[lang];
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setDense(!dense)} title="Density" style={{
          ...
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
            <line x1="2" y1={dense ? 4 : 4} x2="12" y2={dense ? 4 : 4} />
...
const ActiveFilterChips = ({ lang, filters, setFilters }) => {
  const c = window.SEARCH_COPY[lang];
  const chips = [];
...
Object.assign(window, { SearchHero, ResultsHeader, ActiveFilterChips, EmptyState });
