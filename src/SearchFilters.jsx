...
const SearchFilters = ({ lang, filters, setFilters, totalCount, visibleCount }) => {
  ...
  return (
    <aside style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {['verified','self','unknown'].map((s) => (
            <CheckRow key={s} on={(filters.halal||[]).includes(s)} onClick={() => toggle('halal', s)}>
              <window.StatusDot status={s} size={8} />
        ...
        <ChipRow>
          {[4, 4.5].map((r) => (
            <Chip key={r} on={filters.rating === r} onClick={() => setOne('rating', r)}>
              <window.Icon.star size={11} /> <span style={{ marginLeft: 4 }}>{c.ratingLabels[r]}</span>
            </Chip>
          ))}
        </ChipRow>
      </FilterGroup>
      ...
      <div style={{
        ...
        <span style={{ color: 'var(--accent)', fontWeight: 700 }}>{visibleCount}</span> {c.showingOf(visibleCount, totalCount).split(' ').slice(1).join(' ')}
      </div>
    </aside>
  );
};
...
const Chip = ({ on, onClick, children, bold }) => (
...
const CheckRow = ({ on, onClick, children }) => (
  <button onClick={onClick} style={{
...
const SwitchRow = ({ on, onClick, label }) => (
  <button onClick={onClick} style={{
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
...
const DistrictGrid = ({ lang, value, onToggle }) => (
